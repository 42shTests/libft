...
static void				get_n_length(intmax_t n, size_t *len)
{
	if (n == 0)
		*len = 1;
	else
...
char					*ft_printf_fill_b_intmax(t_printf_args *el, intmax_t n)
{
	char				*str;
	...
	el->acc_d = 1;
	el->acc = sizeof(intmax_t) * CHAR_BIT - 1 + sizeof(intmax_t) - 1;
	el->len = el->acc + 1;
	if (el->wi != 0 && el->acc + 1 < el->wi)
		el->len = el->wi;
	else
